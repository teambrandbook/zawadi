...
export default function WipeButton({ href, label, className = "", variant = "primary", showIcon = true }: WipeButtonProps) {
    ...
    return (
        <Link href={href} className={`${className} block w-fit`}>
            <div 
                ref={containerRef}
                className={`group relative flex items-center ${showIcon ? "justify-between pl-8 pr-1.5 min-w-[12rem]" : "justify-center px-10 md:px-14 text-center"} ${bgClass} rounded-full shadow-xl w-fit overflow-hidden cursor-pointer h-12 md:h-14`}
            ...
            >
                ...
                <div className={`relative z-10 flex-1 h-full flex items-center ${!showIcon ? "justify-center" : ""}`}>
                    <div className="relative w-full">
                        ...
                        <span className={`label-fade-1 font-mulish font-bold text-[13px] md:text-[15px] uppercase tracking-widest ${textClass} block opacity-100 ${!showIcon ? "text-center" : ""}`}>
                            {label}
                        </span>

                        {/* Layer 2: Revealed Text (High intensity discovery) */}
                        <span className={`label-reveal absolute inset-0 font-mulish font-bold text-[13px] md:text-[15px] uppercase tracking-widest ${textClass} block z-10 ${!showIcon ? "text-center" : ""}`} style={{ clipPath: "inset(0 100% 0 0)" }}>
                            {label}
                        </span>
                    </div>
                </div>
                ...
                {showIcon && (
                    <div className="flex items-center justify-end shrink-0 ml-4 pointer-events-none">
                        <div className={`icon-circle w-9 h-9 md:w-11 md:h-11 ${circleBgClass} rounded-full flex items-center justify-center relative z-20 shadow-md`}>
                            <svg className="h-4 w-4 md:h-5 md:w-5" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
