        ...
        return (
          <article key={item.id} className="rounded-xl border border-[#DFDFDF] bg-white p-4">
            <div className="flex items-start justify-between">
              ...
              <p className="text-[11px] text-[#9F8151]">{item.label}</p>
            </div>

            <p
              className={
                isNameCard
                  ? "mt-3 min-h-10 break-words text-[22px] font-semibold leading-tight text-[#0A4833]"
                  : "mt-3 text-[30px] font-semibold leading-none text-[#0A4833]"
              }
              title={item.value}
            >
              {item.value}
            </p>
            <p className="mt-1.5 text-xs text-[#7C9B90]">{item.subText}</p>
          </article>
        );
      })}
    </section>
