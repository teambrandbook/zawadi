...
export default function CreateRoleHeader({ onSave, loading, isEditMode = false }: Props) {
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div>
        <h1 className="text-4xl font-bold leading-none text-[#06402B]">
          {isEditMode ? "Edit Role" : "Create Role"}
        </h1>
        <p className="mt-2 text-base leading-6 text-[#5C8A79]">
          {isEditMode
            ? "Update this admin role, configure permissions, and control platform access securely."
      ...
      >
        <Save size={16} />
        <span>{loading ? "Saving..." : isEditMode ? "Update Role" : "Save Changes"}</span>
      </button>
    </div>
  );
}
